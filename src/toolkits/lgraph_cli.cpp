...
enum class OutputFormat { TABLE = 0, CSV, JSON };
  ...
  while (true) {
    ...
    if (msg.type() == typeid(std::optional<bolt::Record>)) {
      ...
      if (values.size() != header.value().size()) {
        LOG_FATAL(
            "mismatched data, header column size: {}, "
            "record column size: {}",
            header.value().size(), values.size());
      }
...
std::string ReadStatement() {
  ...
  while (true) {
    char* line = linenoise(
        is_multiline ? "      -> "
                     : fmt::format("TuGraph[{}]> ", FLAGS_graph).c_str());
    if (line == nullptr) {
      std::exit(0);
    }
    std::unique_ptr<char[], void (*)(char*)> auto_free_line(
        line, [](char* p) { free(p); });
    if (line[0] == '\0') {
      continue;
    }
...
int main(int argc, char** argv) {
  ...
  try {
    asio::connect(socket,
                  resolver.resolve(FLAGS_ip, std::to_string(FLAGS_port)));
    asio::write(socket, asio::const_buffer(handshake, sizeof(handshake)));
    uint8_t accepted_version[4];
    asio::read(socket,
               asio::buffer(accepted_version, sizeof(accepted_version)));
    ...
    ps.AppendHello(meta);
    asio::write(socket, asio::const_buffer(ps.ConstBuffer().data(),
                                           ps.ConstBuffer().size()));
    auto msg = ReadMessage(socket, hydrator);
