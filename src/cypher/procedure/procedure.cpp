...
std::vector<Procedure> global_procedures = {
    Procedure("log.setLevel", LogFunc::SetLevel,
              Procedure::SIG_SPEC{{"level", {0, ProcedureResultType::Value}}},
              Procedure::SIG_SPEC{}),
    Procedure("log.queryLog", LogFunc::QueryLog,
    ...
    Procedure("db.labels", BuiltinProcedure::DbLabels, Procedure::SIG_SPEC{},
              Procedure::SIG_SPEC{{"label", {0, ProcedureResultType::Value}}}),
    Procedure("db.relationshipTypes", BuiltinProcedure::DbRelationshipTypes,
              Procedure::SIG_SPEC{},
              Procedure::SIG_SPEC{
    ...
    Procedure(
        ...
        BuiltinProcedure::DbIndexFullTextQueryNodes,
        Procedure::SIG_SPEC{{"index_name", {0, ProcedureResultType::Value}},
                            {"query", {1, ProcedureResultType::Value}}},
        Procedure::SIG_SPEC{{"node", {0, ProcedureResultType::Node}},
                            {"score", {1, ProcedureResultType::Value}}}),
    Procedure(
        ...
        BuiltinProcedure::DbIndexFullTextDeleteIndex,
        Procedure::SIG_SPEC{{"index_name", {0, ProcedureResultType::Value}}},
        Procedure::SIG_SPEC{}),
    Procedure("db.index.fulltext.applyWal",
              BuiltinProcedure::DbIndexFullTextApplyWal, Procedure::SIG_SPEC{},
              Procedure::SIG_SPEC{}),
    Procedure(
        "db.index.vector.createNodeIndex",
        BuiltinProcedure::DbIndexVectorCreateNodeIndex,
        Procedure::SIG_SPEC{{"index_name", {0, ProcedureResultType::Value}},
                            {"label", {1, ProcedureResultType::Value}},
    ...
    Procedure("db.index.vector.applyWal",
              BuiltinProcedure::DbIndexVectorApplyWal, Procedure::SIG_SPEC{},
              Procedure::SIG_SPEC{}),
    Procedure(
        "db.index.vector.deleteIndex",
        BuiltinProcedure::DbIndexVectorDeleteIndex,
        Procedure::SIG_SPEC{{"index_name", {0, ProcedureResultType::Value}}},
        Procedure::SIG_SPEC{}),
    Procedure(
...
void LogFunc::SetLevel(cypher::RTContext *ctx, const cypher::Record *record,
  ...
  std::set<std::string> levels = {"trace",   "debug", "info",
...
void AlgoFunc::PageRank(cypher::RTContext *ctx, const cypher::Record *record,
  ...
  auto vertex_num = 0;
  for (auto viter = ctx->txn_->NewVertexIterator(); viter->Valid();
       viter->Next()) {
  ...
  while (num_iterations > 0) {
    for (auto viter = ctx->txn_->NewVertexIterator(); viter->Valid();
      ...
      for (auto eiter =
               vertex.NewEdgeIterator(graphdb::EdgeDirection::INCOMING, {}, {});
...
void BuiltinProcedure::DbmsProcedures(
  ...
  CYPHER_THROW_ASSERT(pp && pp->ContainsYieldItem("name") &&
                      pp->ContainsYieldItem("signature"));
  ...
  std::unordered_map<
      std::string,
      std::function<void(const Procedure &, std::vector<ProcedureResult> &)>>
      lmap = {{"name",
               [](const Procedure &p, std::vector<ProcedureResult> &r) {
                 r.emplace_back(Value(p.proc_name));
               }},
              {"signature",
               [](const Procedure &p, std::vector<ProcedureResult> &r) {
                 r.emplace_back(Value(p.Signature()));
               }}};
  for (auto &p : global_procedures) {
    std::vector<ProcedureResult> r;
...
void BuiltinProcedure::DbLabels(
...
void BuiltinProcedure::DbRelationshipTypes(
    cypher::RTContext *ctx, const cypher::Record *record,
    const cypher::VEC_EXPR &args, const cypher::VEC_STR &yield_items,
    std::vector<std::vector<ProcedureResult>> *records) {
  CYPHER_ARG_CHECK(args.empty(),
...
void BuiltinProcedure::DbIndexVectorCreateNodeIndex(
  ...
  CYPHER_ARG_CHECK(
      args.size() == 4,
      fmt::format("Function requires 4 arguments, but {} are "
                  "given. Usage: db.index.vector.createNodeIndex(index_name, "
  ...
  if (parameter.count("hnsw_ef_construction")) {
    hnsw_ef_construction =
        (int)parameter.at("hnsw_ef_construction").AsInteger();
  }
  CYPHER_ARG_CHECK(
      (hnsw_ef_construction <= 1000 && hnsw_ef_construction >= hnsw_m),
      "hnsw.efConstruction should be an integer in the range [hnsw.m,1000]");
  LOG_INFO(
      "Create node vector index, name:{}, label:{}, property:{}, parameter:{}",
...
void BuiltinProcedure::DbIndexVectorKnnSearchNodes(
    cypher::RTContext *ctx, const Record *record, const VEC_EXPR &args,
    const VEC_STR &yield_items,
    std::vector<std::vector<ProcedureResult>> *records) {
  CYPHER_ARG_CHECK(
      args.size() == 3,
      fmt::format("Function requires 3 arguments, but {} are "
                  "given. Usage: db.index.vector.knnSearchNodes(index_name, "
                  "query, parameter)",
                  args.size()))
  CYPHER_ARG_CHECK(args[0].IsString(), "index_name type should be String")
  ...
  for (auto viter = ctx->txn_->QueryVertexByKnnSearch(index_name, vectors,
                                                      top_k, ef_search);
       viter->Valid(); viter->Next()) {
    std::vector<ProcedureResult> r;
    for (auto &yield : yield_items) {
...
void BuiltinProcedure::DbIndexFullTextCreateNodeIndex(
  ...
  CYPHER_ARG_CHECK(
      args.size() == 3,
      fmt::format("Function requires 3 arguments, but {} are "
                  "given. Usage: db.index.fulltext.createNodeIndex(index_name, "
                  "labels, properties)",
                  args.size()))
  CYPHER_ARG_CHECK(args[0].IsString(), "index_name type should be String")
...
void BuiltinProcedure::DbIndexFullTextQueryNodes(
  ...
  CYPHER_ARG_CHECK(
      args.size() == 3,
      fmt::format("Function requires 3 arguments, but {} are "
                  "given. Usage: db.index.fulltext.queryNodes(index_name, "
                  "query, top_n)",
                  args.size()))
  CYPHER_ARG_CHECK(args[0].IsString(), "index_name type should be String")
  ...
  for (const auto &index : graphdb->meta_info().GetVertexFullTextIndexes()) {
    ...
    for (auto &yield : yield_items) {
      if (yield == "name") {
        ...
        r.emplace_back(Value::StringArray(
            {index->meta().labels().begin(), index->meta().labels().end()}));
      } else if (yield == "properties") {
        r.emplace_back(Value::StringArray({index->meta().properties().begin(),
                                           index->meta().properties().end()}));
      } else if (yield == "otherInfo") {
