    ...
    switch (type) {
      ...
      case NODE:
        return (EqualNull() && rhs.EqualNull()) ||
               (type == rhs.type && node && rhs.node &&
                node->vertex_ == rhs.node->vertex_);
      case RELATIONSHIP:
        return (EqualNull() && rhs.EqualNull()) ||
...
struct Record {
  ...
  Record(size_t size, const SymbolTable *sym_tab) : symbol_table(sym_tab) {
    values.resize(size);
  }

  void AddConstant(const Value &value) { values.emplace_back(value); }
