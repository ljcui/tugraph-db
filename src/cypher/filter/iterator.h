...
class LabelVertexIterator {
 ...
 public:
    ...
    lgraph::VertexId GetId() const {
        if (!IsValid()) throw lgraph_api::InternalError("get id of invalid LabelVertexIterator");
        return _it->GetId();
    }
...
class TypeEdgeIterator {
 ...
 public:
    ...
    lgraph::EdgeUid GetUid() const {
        if (!IsValid()) throw lgraph_api::InternalError("get uid of invalid EIter");
        return _eit->GetUid();
    }

    lgraph::VertexId GetSrc() const {
        if (!IsValid()) throw lgraph_api::InternalError("get src of invalid EIter");
        return _eit->GetSrc();
    }

    lgraph::VertexId GetDst() const {
        if (!IsValid()) throw lgraph_api::InternalError("get dst of invalid EIter");
        return _eit->GetDst();
    }

    lgraph::LabelId GetLabel() const {
        if (!IsValid()) throw lgraph_api::InternalError("get label of invalid EIter");
        return _eit->GetLabelId();
    }

    lgraph::EdgeId GetEdgeId() const {
        if (!IsValid()) throw lgraph_api::InternalError("get eid of invalid EIter");
