...
Argument::Argument(const SymbolTable *sym_tab)
  ...
  for (auto &a : sym_tab->symbols) {
    if (a.second.scope == SymbolNode::ARGUMENT) {
      ordered_alias.emplace(a.second.id,
                            std::make_pair(a.first, a.second.type));
    }
  }
...
void Argument::Receive(std::shared_ptr<Record> *input_record,
  ...
  for (auto &a : args_) {
    ...
    switch (input.type) {
      case Entry::CONSTANT:
        ...
        break;
      case Entry::NODE:
        /* Note: The input may be invalid, such as nodes produced by OPTIONAL
         * MATCH. Set the record invalid in that cases. */
      ...
      case Entry::RELATIONSHIP:
        ...
        record->values[a.rec_idx].relationship->edge_ =
            input.relationship->edge_;
