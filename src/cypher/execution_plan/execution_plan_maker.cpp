...
geax::frontend::GEAXErrorCode ExecutionPlanMaker::Build(
    geax::frontend::AstNode* astNode, OpBase*& root, RTContext* ctx) {
  ctx_ = ctx;
  cur_types_.clear();
  cur_pattern_graph_ = -1;
  auto ret =
      std::any_cast<geax::frontend::GEAXErrorCode>(astNode->accept(*this));
...
void ExecutionPlanMaker::_UpdateStreamRoot(OpBase* new_root, OpBase*& root) {
  if (root) {
    /* The new root should have no parent, but may have children if we've
     * constructed a chain of traversals/scans. */
    ...
    while (!tail->children.empty()) {
      if (tail->children.size() > 1 ||
          tail->type == OpType::CARTESIAN_PRODUCT ||
...
OpBase* ExecutionPlanMaker::_SingleBranchConnect(
    const std::vector<OpBase*>& ops) {
...
std::any ExecutionPlanMaker::visit(geax::frontend::PathChain* node) {
  ...
  // For the exists pattern query, SemiAllpy is simulated by combining Limit
  // Optional and ExpandAll operators. e.g MATCH (n {name:'Rachel
  // Kempson'}),(m:Person) RETURN exists((n)-[:MARRIED]->(m)) The resulting
  // execution plan is as follows Produce Results
  ...
  if (ClauseGuard::InClause(geax::frontend::AstNodeType::kExists, cur_types_)) {
    ...
    if
    (ClauseGuard::InClause(geax::frontend::AstNodeType::kPrimitiveResultStatement,
                              cur_types_)) {
  ...
  // The handling here is for the Match Clause, other situations need to be
  // considered. The current design of the Visitor results in different logic
  // for handling PathChain in different Clauses (Match, Create, and even
  ...
  if (auto op = _SingleBranchConnect(expand_ops)) {
    ...
    // Handle the case where multiple PathChains are connected, with special
    // handling for cases where multiple PathChains are mutually independent.
    // Considering that _UpdateStreamRoot connections are more complex and
    // NodeScan cannot be connected directly, this determination requires
    ...
    && pattern_graph_root_[cur_pattern_graph_]->type != OpType::UNWIND)) {
            ...
            pattern_graph_root_[cur_pattern_graph_] = connection;
                   ...
                   pattern_graph_root_[cur_pattern_graph_]->type ==
    OpType::UNWIND &&
        ...
        // For the associated case, currently handled through ast node rewriter,
    but this has
...
std::any ExecutionPlanMaker::visit(geax::frontend::Node* node) {
  ...
  auto& pattern_graph = pattern_graphs_[cur_pattern_graph_];
  if (!ClauseGuard::InClause(geax::frontend::AstNodeType::kPathChain,
                             cur_types_)) {
    std::vector<OpBase*> expand_ops;
...
std::any ExecutionPlanMaker::visit(geax::frontend::ElementFiller* node) {
  ...
  } else if (ClauseGuard::InClause(geax::frontend::AstNodeType::kEdge,
                                   cur_types_)) {
...
std::any ExecutionPlanMaker::visit(geax::frontend::PathModePrefix*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::PathSearchPrefix*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::SingleLabel* node) {
  if (ClauseGuard::InClause(geax::frontend::AstNodeType::kNode, cur_types_)) {
    ...
    return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
  } else if ((ClauseGuard::InClause(geax::frontend::AstNodeType::kIsLabeled,
                                    cur_types_))) {
    return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
  }
  NOT_SUPPORT();
...
std::any ExecutionPlanMaker::visit(geax::frontend::LabelOr*) { NOT_SUPPORT(); }
...
std::any ExecutionPlanMaker::visit(geax::frontend::TableFunctionClause*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ReadConsistency*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::AllowAnonymousTable*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::OpConcurrent*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::EdgeOnJoin*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetAllProperties*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::UpdateProperties*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetLabel*) { NOT_SUPPORT(); }

std::any ExecutionPlanMaker::visit(geax::frontend::SetSingleProperty*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetSchemaClause*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetGraphClause*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetTimeZoneClause*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SetParamClause*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ResetAll*) { NOT_SUPPORT(); }

std::any ExecutionPlanMaker::visit(geax::frontend::ResetSchema*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ResetTimeZone*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ResetGraph*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ResetParam*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::BSafeEqual*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::BBitLeftShift*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BBitRightShift*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::If*) { NOT_SUPPORT(); }
...
std::any ExecutionPlanMaker::visit(geax::frontend::MatchCase*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::MultiCount*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::Windowing*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::VDate*) { NOT_SUPPORT(); }

std::any ExecutionPlanMaker::visit(geax::frontend::VDatetime*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::VDuration*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::VTime*) { NOT_SUPPORT(); }

std::any ExecutionPlanMaker::visit(geax::frontend::VNull*) { NOT_SUPPORT(); }
...
std::any ExecutionPlanMaker::visit(geax::frontend::IsDirected*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::IsNormalized*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::IsSourceOf*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::IsDestinationOf*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::AllDifferent*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::Exists*) { NOT_SUPPORT(); }

std::any ExecutionPlanMaker::visit(geax::frontend::ExplainActivity*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SessionActivity*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::FullTransaction*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::StartTransaction*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::CommitTransaction*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::RollBackTransaction*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SessionSet*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::SessionReset*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::SchemaFromPath*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BindingValue*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BindingGraph*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BindingTable*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BindingTableInnerQuery*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::BindingTableInnerExpr*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(
    geax::frontend::StandaloneCallStatement* node) {
...
std::any ExecutionPlanMaker::visit(geax::frontend::JoinRightPart*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(
    geax::frontend::CompositeQueryStatement* node) {
  auto head = node->head();
  ACCEPT_AND_CHECK_WITH_ERROR_MSG(head);
  if (!node->body().empty()) {
    ...
    for (auto statement : node->body()) {
      ...
      if (cur_pattern_graph_ == pattern_graph_size_ - 1) {
        op_union->AddChild(
            pattern_graph_root_[cur_pattern_graph_]->children[0]);
      } else {
        op_union->AddChild(pattern_graph_root_[cur_pattern_graph_]);
      }
    }
...
std::any ExecutionPlanMaker::visit(
    geax::frontend::AmbientLinearQueryStatement* node) {
...
std::any ExecutionPlanMaker::visit(geax::frontend::SelectStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::FocusedQueryStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::FocusedResultStatement*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::MatchStatement* node) {
  ...
  if (node->statementMode().has_value() &&
      node->statementMode().value() ==
          geax::frontend::StatementMode::kOptional) {
...
std::any ExecutionPlanMaker::visit(
    geax::frontend::CallProcedureStatement* node) {
  auto procedure = node->procedureCall();
  ACCEPT_AND_CHECK_WITH_ERROR_MSG(procedure);
  return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
}

std::any ExecutionPlanMaker::visit(geax::frontend::InlineProcedureCall*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::NamedProcedureCall* node) {
  ...
  std::vector<OpBase*> expand_ops;
  auto standalone_call_op =
      new GqlStandaloneCall(name, node->args(), node->yield(),
                            pattern_graphs_[cur_pattern_graph_].symbol_table);
...
std::any ExecutionPlanMaker::visit(geax::frontend::ForStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(
    geax::frontend::PrimitiveResultStatement* node) {
  ...
  for (auto& item : items) {
    ...
    if (expr->type() == geax::frontend::AstNodeType::kRef) {
      ...
      auto it =
          pattern_graphs_[cur_pattern_graph_].symbol_table.symbols.find(name);
      if (it ==
  ...
  // TODO(lingsu): check whether the orderby key appears in the symbol table or
  // return items, and GetField type should complement the return items
  for (auto order_by_field : node->orderBys()) {
    for (size_t i = 0; i < items.size(); ++i) {
      if (auto order_by_ref =
              dynamic_cast<geax::frontend::Ref*>(order_by_field->field())) {
        auto field_name = std::get<0>(items[i]);
        if (order_by_ref->name() == field_name) {
          order_by_items.emplace_back(
              std::make_pair(i, !order_by_field->order()));
          break;
        }
      } else if (auto field = dynamic_cast<geax::frontend::GetField*>(
                     order_by_field->field())) {
        if (auto order_by_ref =
                dynamic_cast<geax::frontend::Ref*>(field->expr())) {
          auto field_name = std::get<0>(items[i]);
  ...
  bool has_aggregation = false;
  {
    ...
    for (auto& item : items) {
      ArithExprNode ae(std::get<1>(item),
                       pattern_graphs_[cur_pattern_graph_].symbol_table);
      auto alias = std::get<0>(item);
    ...
    if (has_aggregation) {
      ops.emplace_back(new Aggregate(
          ...
          noneaggr_item_names, item_names,
          &pattern_graphs_[cur_pattern_graph_].symbol_table,
          result_info_.header));
    }
  }
  if (!has_aggregation) {
    ...
    ops.emplace_back(new Project(
        arith_items, &pattern_graphs_[cur_pattern_graph_].symbol_table));
  }
...
std::any ExecutionPlanMaker::visit(geax::frontend::CatalogModifyStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(
    geax::frontend::LinearDataModifyingStatement* node) {
...
std::any ExecutionPlanMaker::visit(geax::frontend::InsertStatement* node) {
  ...
  _UpdateStreamRoot(op, pattern_graph_root_[cur_pattern_graph_]);
  result_info_.header.colums.clear();
  result_info_.header.colums.emplace_back("<SUMMARY>", "", false);
  return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
}

std::any ExecutionPlanMaker::visit(geax::frontend::ReplaceStatement*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::MergeStatement* node) {
  auto& pattern_graph = pattern_graphs_[cur_pattern_graph_];
  auto op = new OpGqlMerge(node->onMatch(), node->onCreate(),
                           node->pathPattern(), &pattern_graph);
  _UpdateStreamRoot(op, pattern_graph_root_[cur_pattern_graph_]);
  result_info_.header.colums.clear();
  result_info_.header.colums.emplace_back("<SUMMARY>", "", false);
  return geax::frontend::GEAXErrorCode::GEAX_SUCCEED;
...
std::any ExecutionPlanMaker::visit(geax::frontend::OtherWise*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::Intersect*) {
  NOT_SUPPORT();
}
...
std::any ExecutionPlanMaker::visit(geax::frontend::KillStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::ManagerStatement*) {
  NOT_SUPPORT();
}

std::any ExecutionPlanMaker::visit(geax::frontend::UnwindStatement* node) {
  ArithExprNode exp(node->list(),
                    pattern_graphs_[cur_pattern_graph_].symbol_table);
  auto unwind = new Unwind(exp, node->variable(),
                           &pattern_graphs_[cur_pattern_graph_].symbol_table);
...
std::any ExecutionPlanMaker::visit(geax::frontend::InQueryProcedureCall* node) {
  std::string name = std::get<std::string>(node->name());
  std::vector<OpBase*> expand_ops;
  auto op = new GqlInQueryCall(name, node->args(), node->yield(),
                               &pattern_graphs_[cur_pattern_graph_]);
...
std::any ExecutionPlanMaker::visit(geax::frontend::DummyNode*) {
  NOT_SUPPORT();
}
