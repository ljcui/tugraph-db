...
static inline bool IsNumeric(const Value &x) {
  return x.IsInteger() || x.IsDouble();
}

static inline bool IsTimeRelated(const Value &x) {
  return x.IsTime() || x.IsDateTime() || x.IsLocalTime() || x.IsDate() ||
         x.IsLocalDateTime();
}
...
static inline void AddList(Value &ret, const Value &x) {
  if (x.IsArray())
    ret.AsArray().insert(ret.AsArray().end(), x.AsArray().begin(),
                         x.AsArray().end());
  else
...
static inline void AddString(Value &ret, const Value &x) {
  if (x.IsBool())
    THROW_CODE(CypherException, "Type mismatch: cannot add BOOL with STRING");
  if (x.IsString()) {
...
[[maybe_unused]] static Value Add(const Value &x, const Value &y) {
...
[[maybe_unused]] static Value Sub(const Value &x, const Value &y) {
  if (!((IsNumeric(x) || x.IsNull()) && (IsNumeric(y) || y.IsNull())) &&
      !((IsTimeRelated(x) || IsDuration(x)) && IsDuration(y)))
    THROW_CODE(CypherException,
               "Type mismatch: expect Integer or Float or TimeRelated or "
               "Duration in sub expr");
  if (x.IsNull() || y.IsNull()) return {};
  Value ret;
...
[[maybe_unused]] static Value Mul(const Value &x, const Value &y) {
  if (!((IsNumeric(x) || x.IsNull() || IsDuration(x)) &&
        (IsNumeric(y) || y.IsNull())))
    THROW_CODE(
        CypherException,
        "Type mismatch: expect Integer or Float or Duration in mul expr");
  if (x.IsNull() || y.IsNull()) return {};
  Value ret;
  if (x.IsInteger() && y.IsInteger()) {
    ret = Value(x.AsInteger() * y.AsInteger());
...
[[maybe_unused]] static Value Div(const Value &x, const Value &y) {
  if (!((IsNumeric(x) || x.IsNull() || IsDuration(x)) &&
        (IsNumeric(y) || y.IsNull())))
    THROW_CODE(
        CypherException,
        "Type mismatch: expect Integer or Float or Duration in div expr");
  if (x.IsNull() || y.IsNull()) return {};
  Value ret;
  if (x.IsInteger() && y.IsInteger()) {
...
[[maybe_unused]] static Value Mod(const Value &x, const Value &y) {
  if (!((IsNumeric(x) || x.IsNull()) && (IsNumeric(y) || y.IsNull())))
    THROW_CODE(CypherException,
               "Type mismatch: expect Integer or Float in mul expr");
  if (x.IsNull() || y.IsNull()) return {};
  Value ret;
  if (x.IsInteger() && y.IsInteger()) {
    int64_t x_n = x.AsInteger();
...
[[maybe_unused]] static Value Pow(const Value &x, const Value &y) {
  if (!((IsNumeric(x) || x.IsNull()) && (IsNumeric(y) || y.IsNull())))
    THROW_CODE(CypherException,
               "Type mismatch: expect Integer or Float in mul expr");
  if (x.IsNull() || y.IsNull()) return {};
...
struct BuiltinFunction {
  ...
  static Value DateTimeFromEpoch(RTContext *ctx, const Record &record,
                                 const std::vector<ArithExprNode> &args);
  static Value DateTimeFromEpochMillis(RTContext *ctx, const Record &record,
                                       const std::vector<ArithExprNode> &args);
 ...
 private:
  ...
  _TimeUnitTruncate(RTContext *ctx, const Record &record,
                    const std::vector<ArithExprNode> &args);
};
...
struct ArithOperandNode {
  ...
  void SetVariable(const bool &hasMapFieldName = false,
  ...
  std::string ToString() const {
    ...
    if (type == AR_OPERAND_VARIADIC) {
      ...
      if (!variadic.entity_prop.empty())
        str.append(".").append(variadic.entity_prop);
...
struct ArithOpNode {
  ...
  RegisterFuncs() {
    ...
    ae_registered_funcs.emplace("localdatetime",
                                BuiltinFunction::LocalDateTime);
    ...
    ae_registered_funcs.emplace("tofloat32list",
                                BuiltinFunction::ToFloat32List);
    ...
    ae_registered_funcs.emplace("localtime.truncate",
                                BuiltinFunction::LocalTimeTruncate);
    ae_registered_funcs.emplace("time.truncate", BuiltinFunction::TimeTruncate);
    ae_registered_funcs.emplace("duration.between",
                                BuiltinFunction::DurationBetween);
    ae_registered_funcs.emplace("duration.inseconds",
                                BuiltinFunction::DurationInSeconds);
    ae_registered_funcs.emplace("duration.indays",
                                BuiltinFunction::DurationInDays);
    ae_registered_funcs.emplace("duration.inmonths",
                                BuiltinFunction::DurationInMonths);
    ae_registered_funcs.emplace("duration.between",
                                BuiltinFunction::TimeTruncate);
    ...
    ae_registered_funcs.emplace("date.truncate", BuiltinFunction::DateTruncate);
  ...
  static std::unordered_map<std::string,
                            std::function<std::shared_ptr<AggCtx>()>>
  RegisterAggFuncs() {
    std::unordered_map<std::string, std::function<std::shared_ptr<AggCtx>()>>
        agg_registered_funcs;
    agg_registered_funcs.emplace("sum",
                                 [] { return std::make_shared<SumAggCtx>(); });
    agg_registered_funcs.emplace("avg",
                                 [] { return std::make_shared<AvgAggCtx>(); });
    agg_registered_funcs.emplace("max",
                                 [] { return std::make_shared<MaxAggCtx>(); });
    agg_registered_funcs.emplace("min",
                                 [] { return std::make_shared<MinAggCtx>(); });
    agg_registered_funcs.emplace(
        "count", [] { return std::make_shared<CountAggCtx>(); });
    agg_registered_funcs.emplace(
        "count(*)", [] { return std::make_shared<CountStarAggCtx>(); });
    agg_registered_funcs.emplace(
        "collect", [] { return std::make_shared<CollectAggCtx>(); });
    ...
    agg_registered_funcs.emplace(
        "stdev", [] { return std::make_shared<StDevAggCtx>(); });
    agg_registered_funcs.emplace(
        "stdevp", [] { return std::make_shared<StDevPAggCtx>(); });
    agg_registered_funcs.emplace(
        "variance", [] { return std::make_shared<VarianceAggCtx>(); });
    agg_registered_funcs.emplace(
        "variancep", [] { return std::make_shared<VariancePAggCtx>(); });
...
struct ArithExprNode {
  ...
  enum ArithExprType {
    AR_EXP_OPERAND,
    AR_EXP_OP,
    // todo (kehuang): AstExpr is currently temporarily used as a type of
    // ArithExpr, and will be replaced with AstExpr in the future.
    AR_AST_EXP,
  } type;
  ...
  void SetOperandVariable(ArithOperandNode::ArithOperandType operand_type,
                          const bool &hasMapFieldName = false,
                          const std::string &value_alias = "",
                          const std::string &map_field_name = "") {
  ...
  void Aggregate(RTContext *ctx, const Record &record) {
    if (type == AR_EXP_OP) {
      if (op.type == ArithOpNode::AR_OP_AGGREGATE) {
        ...
        // Trying to understand: clang's side-effect warnings for typeid on a
        // polymorphic object
  ...
  bool ContainsAggregation() const {
    if (type == AR_EXP_OP) {
      ...
      return agg_expr_detector.Validate() &&
             agg_expr_detector.HasValidAggFunc();
    }
    return false;
  }
