...
void PythonPluginManagerImpl::LoadPlugin(const std::string& user, const std::string& name,
    ...
    case python_plugin::TaskOutput::ErrorCode::INTERNAL_ERR:
        throw lgraph_api::InternalError(FMA_FMT("Unexpected error occured when loading plugin: [{}]", output));
    case python_plugin::TaskOutput::ErrorCode::INPUT_ERR:
        throw lgraph_api::InputError(FMA_FMT("Unexpected error occured when loading plugin: [{}]", output));
    default:
        throw lgraph_api::InternalError(FMA_FMT("Unhandled error code [{}].", ec));
    }
}
...
python_plugin::TaskOutput::ErrorCode PythonPluginManagerImpl::CallInternal(
    ...
    while (true) {
        ...
        if (TaskTracker::GetInstance().ShouldKillCurrentTask() || proc->ShouldKill()) {
            ...
            throw lgraph_api::TaskKilled();
        }
        // if process failed, break
        if (!proc->IsAlive()) {
            output = proc->Stderr();
            throw lgraph_api::InternalError(FMA_FMT("Plugin failed unexpectly. Stderr:\n{}", output));
        }
        ...
        if (fma_common::GetTime() - start_time >= timeout) {
            ...
            output = proc->Stderr();
            _marked_processes.insert(std::move(proc));
            throw lgraph_api::PythonPluginTimeout(
                FMA_FMT("Task timed out, timeout = [{}] seconds.", timeout));
        }
    }
    rollback.Cancel();
