...
TEST(VarLenDB, db) {
  ...
  txn->Execute(&rtx,
               "CREATE (rachel:Person {name: 'Rachel Kempson', birthyear: "
               "1910})-[:HAS_CHILD]->(vanessa:Person {name: 'Vanessa "
               "Redgrave', birthyear: 1937})")
      ->Consume();
  txn->Commit();
  txn = graphDB->BeginTransaction();
  auto resultIterator = txn->Execute(
      &rtx, "MATCH p=(n:Person {name: 'Vanessa Redgrave'})<-[]-(m) RETURN p;");
  ...
  for (; resultIterator->Valid(); resultIterator->Next()) {
    ...
    EXPECT_EQ(
        std::any_cast<common::Node>(path[0].data).properties["name"].AsString(),
        "Vanessa Redgrave");
    EXPECT_FALSE(path[1].is_node);
    EXPECT_EQ(std::any_cast<common::Relationship>(path[1].data).type,
              "HAS_CHILD");
    EXPECT_TRUE(path[2].is_node);
    EXPECT_EQ(
        std::any_cast<common::Node>(path[2].data).properties["name"].AsString(),
        "Rachel Kempson");
  }
  txn->Commit();
  txn = graphDB->BeginTransaction();
